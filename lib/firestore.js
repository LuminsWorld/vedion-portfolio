...
export async function queryDocs(collPath, filters = []) {
  const parts = collPath.split('/')
  const collId = parts.pop()
  const parentPath = parts.join('/')
  const parent = `projects/${PROJECT}/databases/(default)/documents${parentPath ? '/' + parentPath : ''}`
  ...
  const body = { structuredQuery: { from: [{ collectionId: collId }] } }
  ...
  return rows
    .filter(r => r.document)
    .map(r => ({ id: docId(r.document.name), ...decFields(r.document.fields ?? {}) }))
}
